...
contract("MultiSigWalletAPI", (accounts) => {
  ...
  beforeEach(async () => {
    ...
    vaultFactoryInstance = await VaultProxyBeaconFactory.new(implementationInstance.address, initialSigners, requiredSignatures, vaultOwner, identifier);

    const {logs} = await vaultFactoryInstance.create(initialSigners, requiredSignatures, identifier);
  ...
  it("vault owner should the defined address", async () => {
    assert.equal(await walletInstance.owner(), vaultOwner, "the vault owner should be the defined address");
  });
  ...
  it("Should be able to update the ownership of the vault factory", async () => {
    await vaultFactoryInstance.transferOwnership(signer2, { from: vaultOwner });
  ...
  it("Should be able to update the ownership of the vault", async () => {
    await walletInstance.transferOwnership(signer2, { from: vaultOwner });
    assert.equal(await walletInstance.owner(), signer2, "the forwarder address should be the defined address");
  });
  // it.only("Should update signer", async () => {
  //   await walletInstance.requestSignerChange(signer2,signer6, { from: vaultOwner });
  //   await walletInstance.confirmSignerChange(signer2,signer6, { from: signer2 });

  //   assert(await walletInstance.isSigner(signer2), false, "signer has been removed") // return true
  //   assert(await walletInstance.isSigner(signer6), true, "signer should exists") // return false
