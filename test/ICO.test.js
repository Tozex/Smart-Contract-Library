    ...
    describe('buyTokens', function () {
        describe('validate', function() {
            ...
            it('revert because ICO is already finished', async function () {
                ...
                await expectRevert(this.ico.buyTokens(ONE_TOKEN, {from:investor1}), "ICO.buyTokens: ICO is already finished.");
            });
    
            it('revert because deposit amount is less than min purchase amount', async function () {
                await this.ico.setIco( true, {from:owner});
                await expectRevert(this.ico.buyTokens(ONE_TOKEN, {from:investor1}), "ICO.buyTokens: Failed the amount is not respecting the minimum deposit of ICO");
            });
            ...
            it('revert because ICO contract doesnt have reward token', async function () {
                ...
                await expectRevert(this.ico.buyTokens(TEN_TOKEN, {from:investor1}), "ICO.buyTokens: not enough token to send");
            });
            ...
            it('revert because ICO contract doesnt have enough reward token', async function () {
                ...
                await this.daiToken.approve(this.ico.address, TEN_TOKEN, {from: investor2});
        ...
        describe('buyTokens success', async function () {
            ...
            it('buyTokens', async function () {
                ...
                await this.daiToken.approve(this.ico.address, TEN_TOKEN, {from: investor1});
                await this.ico.buyTokens(TEN_TOKEN, {from:investor1});
                const newBalance = await this.daiToken.balanceOf(masterWallet);
                ...
                expect(userDetail.totalRewardAmount).to.be.bignumber.equal('5000000000000000000');
            ...
            it('buyTokens by 2nd investor (check decimals)', async function () {
                const oldBalance = await this.daiToken.balanceOf(masterWallet);
                await this.daiToken.approve(this.ico.address, new BN('12500000000000000000'), {from: investor2});
                await this.ico.buyTokens(new BN('12500000000000000000'), {from:investor2});
                const newBalance = await this.daiToken.balanceOf(masterWallet);
                const changes = new BN(newBalance.sub(oldBalance));
                expect(changes).to.be.bignumber.equal('12500000000000000000');
                const userDetail = await this.ico.userDetails(investor2);
                expect(userDetail.totalRewardAmount).to.be.bignumber.equal('6250000000000000000');
        ...
        describe('buyTokens - check decimals', function () {
            it('buyTokens by 2nd investor (check decimals)', async function () {
                ...
                this.ico = await ICO.new(masterWallet, this.daiToken.address, this.token.address, new BN('9'), ONE_HUNDRED_TOKENS, TWO_TOKENS, {from: owner});
                await this.token.mint(owner, tokenSupply, {from: owner});
                await this.token.transfer(this.ico.address, ONE_HUNDRED_TOKENS, {from: owner});

                const oldBalance = await this.daiToken.balanceOf(masterWallet);
                await this.daiToken.approve(this.ico.address, new BN('10000000000000000000'), {from: investor2});
                await this.ico.buyTokens(new BN('10000000000000000000'), {from:investor2});
                const newBalance = await this.daiToken.balanceOf(masterWallet);
                const changes = new BN(newBalance.sub(oldBalance));
                expect(changes).to.be.bignumber.equal('10000000000000000000');
                const userDetail = await this.ico.userDetails(investor2);
                expect(userDetail.totalRewardAmount).to.be.bignumber.equal('5000000000');
                expect(userDetail.depositAmount).to.be.bignumber.equal('10000000000000000000');
                expect(userDetail.withdrawAmount).to.be.bignumber.equal('0');
            });
        }
